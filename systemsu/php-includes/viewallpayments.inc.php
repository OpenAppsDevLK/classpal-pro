...
$stmt1119 = $db->prepare("SELECT cp_users.id, cp_users.firstname, cp_users.lastname, cp_userpermission.permission_id, cp_userpermission.uid, cp_userpermission.OnOff  FROM `cp_users` INNER JOIN `cp_userpermission` ON cp_users.id = cp_userpermission.uid WHERE cp_userpermission.uid = {$_SESSION['user_id']} AND cp_userpermission.permission_id = 1119" ); 
$stmt1119->bind_result($cp_users_id, $cp_users_firstname, $cp_users_lastname, $cp_userpermission_permission_id, $cp_userpermission_uid, $cp_userpermission_OnOff);
$stmt1119->execute();
...
$sql = "SELECT cp_payments.pay_id, cp_payments.Pay_stu_studentID, cp_payments.pay_student_name, cp_payments.pay_subj_id, cp_payments.pay_lec_id, cp_payments.pay_paymentdate, cp_payments.pay_paymentmonth, cp_payments.pay_cos_fee, cp_payments.pay_cos_admi, cp_payments.pay_cos_total, cp_lecturers.lec_id, cp_lecturers.lec_name FROM `cp_lecturers` RIGHT JOIN `cp_payments` ON cp_lecturers.lec_id = cp_payments.pay_lec_id WHERE cp_payments.pay_subj_id=$Subj_ID ORDER BY cp_payments.pay_paymentdate DESC $limit";
...
?>
        ...
        <div class="box-body">
         ...
         <!-- Search Form -->       
                ...
                <form style="margin-bottom: 10px;" role="form" method="get" action="" class="form-inline">
                    ...
                    <input style="margin-top: 10px; width: 220px" class="form-control" type="text" name="SearchKey" value="<?php echo $_GET['SearchKey']; ?>" placeholder="Student ID, Name or Paid Month"/>
                    ...
                    <a style="margin-top: 10px;" href="index.php?page=PaymentPending&SubjectID=<?php echo $Subj_ID ?>&SearchKey=<?php echo date('Ym'); ?>" class="btn btn-primary btn-flat" >Payment Pending</a>
                </form>
      ...
      <h4 class="box-title">Total Income: Rs. <?php echo $TotalIncome; ?></h4>
                    ...
                    <tbody>
                      ...
                      <tr>
                         ...
                         <td><?php  echo $row['Pay_stu_studentID'] ?></td>
                         <td><?php echo $row['pay_student_name']  ?></td>
                         <td><?php echo $row['lec_name']  ?></td>
                         <td><?php echo $row['pay_paymentdate']  ?></td>
                         <td><?php  echo $row['pay_paymentmonth'] ?></td>
                         <td><?php echo $row['pay_cos_fee']  ?></td>
                         <td><?php  echo $row['pay_cos_admi'] ?></td>
                         <td><?php echo $row['pay_cos_total']  ?></td>
			...
			<SCRIPT LANGUAGE="JavaScript">
                                    ...
                                    swal({   title: "Are you sure?",   text: "Do you want to delete?",   type: "warning",   showCancelButton: true,   confirmButtonColor: "#DD6B55",   confirmButtonText: "Yes, delete.",   cancelButtonText: "No, Cancel.",   closeOnConfirm: false,   closeOnCancel: false }, function(isConfirm){   if (isConfirm) {  document.myform.action = "actions/deletepayments.php?page=ViewAllPayments&SubjectID=<?php echo $SUBJECT_ID; ?>&PageNo=<?php echo $PNo ?>";  document.myform.submit(); } else {     swal("Cancelled", "Your items are safe :)", "error");   } });
 ...
 <?php
                       ...
                       $sql_2 = "SELECT cp_payments.pay_id, cp_payments.Pay_stu_studentID, cp_payments.pay_student_name, cp_payments.pay_subj_id, cp_payments.pay_lec_id, cp_payments.pay_paymentdate, cp_payments.pay_paymentmonth, cp_payments.pay_cos_fee, cp_payments.pay_cos_admi, cp_payments.pay_cos_total, cp_lecturers.lec_id, cp_lecturers.lec_name FROM `cp_lecturers` RIGHT JOIN `cp_payments` ON cp_lecturers.lec_id = cp_payments.pay_lec_id WHERE (Pay_stu_studentID LIKE '%{$SearchKey}%' OR pay_student_name LIKE '%{$SearchKey}%' OR pay_paymentmonth LIKE '%{$SearchKey}%') AND pay_subj_id=$Subj_ID ORDER BY pay_paymentdate DESC";
        ...
        <?php
               $stmt3 = $db->prepare("SELECT SUM(pay_cos_total) FROM cp_payments WHERE (pay_paymentmonth LIKE '%{$SearchKey}%' OR pay_student_name LIKE '%{$SearchKey}%' OR Pay_stu_studentID LIKE '%{$SearchKey}%') AND pay_subj_id=$Subj_ID ");
               $stmt3->bind_result($TotalIncome);
               $stmt3->execute();
               while ($stmt3->fetch()){
...
<form name="myform2" action="" method="post">
    ...
    <table id="vas_table2" class="table table-hover table-bordered table-responsive">
			...
			<SCRIPT LANGUAGE="JavaScript">
                                    ...
                                    swal({   title: "Are you sure?",   text: "Do you want to delete?",   type: "warning",   showCancelButton: true,   confirmButtonColor: "#DD6B55",   confirmButtonText: "Yes, delete.",   cancelButtonText: "No, Cancel.",   closeOnConfirm: false,   closeOnCancel: false }, function(isConfirm){   if (isConfirm) {  document.myform2.action = "actions/deletepayments.php?page=ViewAllPayments&SubjectID=<?php echo $SUBJECT_ID2; ?>&SearchKey=<?php echo $SearchKey ?>";  document.myform2.submit(); } else {     swal("Cancelled", "Your items are safe :)", "error");   } });
