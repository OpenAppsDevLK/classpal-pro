...
// Select the user and assign permission...          
    $stmt1112 = $db->prepare("SELECT cp_users.id, cp_users.firstname, cp_users.lastname, cp_userpermission.permission_id, cp_userpermission.uid, cp_userpermission.OnOff  FROM `cp_users` INNER JOIN `cp_userpermission` ON cp_users.id = cp_userpermission.uid WHERE cp_userpermission.uid = {$_SESSION['user_id']} AND cp_userpermission.permission_id = 1112");
    $stmt1112->bind_result($cp_users_id, $cp_users_firstname, $cp_users_lastname, $cp_userpermission_permission_id, $cp_userpermission_uid, $cp_userpermission_OnOff);
    $stmt1112->execute();
    ...
    if ($cp_userpermission_OnOff == 0) {
        ...
        ?>
                        ...
                        <form style="margin-bottom: 10px;" role="form" method="get" action="" class="form-inline">
                            ...
                            <input style="margin-top: 10px;" class="form-control" type="text" name="SearchKey" value="<?php echo $_GET['SearchKey']; ?>" placeholder="Student ID|Name|Barcode"/>
...
<!--                                                        <a href="index.php?page=AddStudentPayment&StudentID=<?php //echo $row['stu_studentID']; ?>&StudentName=<?php //echo $row['stu_studentname']; ?>&PageNo=<?php //echo $PNo; ?>&ReceiptNo=<?php //echo $ReceiptNo; ?>" target="_blank" class="btn btn-app ">
                                                    ...
                                                    <td><a href="tel:<?php echo $row['stu_con_mobile1']; ?>"><?php echo $row['stu_con_mobile1']; ?></a></td>
                                                    <td style="text-align: center;"><a class="btn btn-info btn-flat" href="index.php?page=SendSMS&PhoneNub=<?php echo $row['stu_con_mobile1']; ?>"><span class="fa fa-send"></span></a></td>
            ...
            document.querySelector('#swalt').onclick = function setDeleteAction(){
                                        swal({   title: "Are you sure?", text: "Do you want to delete?", type: "warning", showCancelButton: true, confirmButtonColor: "#DD6B55", confirmButtonText: "Yes, delete.", cancelButtonText: "No, Cancel.", closeOnConfirm: false, closeOnCancel: false }, function(isConfirm){   if (isConfirm) {  document.myform.action = "actions/deleteallstu.php?page=ViewAllStudents&PageNo=<?php echo $PNo ?>"; document.myform.submit(); } else {     swal("Cancelled", "It is  safe :)", "error"); } });
            };
            ...
            $sql_2 = "SELECT * FROM cp_students WHERE stu_studentID LIKE '%{$SearchKey}%' OR stu_studentname LIKE '%{$SearchKey}%' OR stu_barcode LIKE '%{$SearchKey}%'";
...
<!--                                                            <a href="index.php?page=AddStudentPayment&StudentID=<?php //echo $row['stu_studentID']; ?>&StudentName=<?php //echo $row['stu_studentname']; ?>&ReceiptNo=<?php //echo $ReceiptNo; ?>" target="_blank" class="btn btn-app ">
                                                                <i class="fa fa-usd"></i> Add Payment
                                                            </a> -->

                                                            <a href="index.php?page=UpdateStudentDetails&StudentID=<?php echo $row['stu_studentID']; ?>&SearchKey=<?php echo $SearchKey; ?>" class="btn btn-app ">
                                                                <i class="fa fa-edit"></i> Edit
                                                            </a> 
