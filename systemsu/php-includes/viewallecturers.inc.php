...
// Select the user and assign permission...          
    $stmt1135 = $db->prepare("SELECT cp_users.id, cp_users.firstname, cp_users.lastname, cp_userpermission.permission_id, cp_userpermission.uid, cp_userpermission.OnOff  FROM `cp_users` INNER JOIN `cp_userpermission` ON cp_users.id = cp_userpermission.uid WHERE cp_userpermission.uid = {$_SESSION['user_id']} AND cp_userpermission.permission_id = 1135");
    $stmt1135->bind_result($cp_users_id, $cp_users_firstname, $cp_users_lastname, $cp_userpermission_permission_id, $cp_userpermission_uid, $cp_userpermission_OnOff);
    $stmt1135->execute();

    while ($stmt1135->fetch()) {
...
// This first query is just to get the total count of rows
    $sql = "SELECT COUNT(id) FROM cp_lecturers";
...
// This is your query again , it is for grabbing just one page worth of rows by applying $limit
    $sql = "SELECT * FROM cp_lecturers ORDER BY lec_name ASC $limit";
...
// Establish the $paginationCtrls variable
    ...
    if ($last != 1) {
        ...
        if ($pagenum > 1) {
            $previous = $pagenum - 1;
            $paginationCtrls .= '<li><a href="index.php?page=ViewAllLecturers&PageNo=1">&laquo;&laquo;</a></li>'
                    . '<li><a href="index.php?page=ViewAllLecturers&PageNo=' . $previous . '">&laquo;</a></li>';

            // Render clickable number links that should appear on the left of the target page number
            for ($i = $pagenum - 2; $i < $pagenum; $i++) {
                if ($i > 0) {
                    $paginationCtrls .= '<li><a href="index.php?page=ViewAllLecturers&PageNo=' . $i . '">' . $i . '</a></li>';
                }
            }
        }
        ...
        if ($pagenum != $last) {
            $next = $pagenum + 1;
            $paginationCtrls .= '<li><a href="index.php?page=ViewAllLecturers&PageNo=' . $next . '">&raquo;</a></li> '
                    . '<li><a href="index.php?page=ViewAllLecturers&PageNo=' . $last . '">&raquo;&raquo;</a></li>'
                    . '</ul>';
        }
    }
    ?>
    ...
    if ($cp_userpermission_OnOff == 0) {
            ...
            <section class="content">
                <div class="box box-primary">
                    ...
                    <div class="box-header with-border">
                        <h3 class="box-title">Lecturer List</h3>
        ...
        ?>
                        ...
                        <form style="margin-bottom: 10px;" role="form" method="get" action="" class="form-inline">
                            <input type="hidden" name="page" value="ViewAllLecturers">
                            ...
                            <a style="margin-top: 10px;" href="index.php?page=ViewAllLecturers&PageNo=1" class="btn btn-success btn-flat" >View All</a>
                        </form>
        ...
        if (!isset($_GET["SearchKey"])) {
                ...
                ?>
                                                ...
                                                <tr>
                                                    <td>
                                                        ...
                                                        <input name="checkbox[]" type="checkbox" style="width:15px; height: 15px; border-radius: 0px;" id="check_list" value="<?php echo $row['id'] ?>" />
                                                        <br>

                                                        <a href="index.php?page=EditLecturer&LecturerID=<?php echo $row['lec_id']; ?>" class="btn btn-app ">
                                                    ...
                                                    <td><a href="tel:<?php echo $row['lec_mob']; ?>"><?php echo $row['lec_mob']; ?></a></td>
            ...
            document.querySelector('#swalt').onclick = function setDeleteAction(){
                                        swal({   title: "Are you sure?", text: "Do you want to delete?", type: "warning", showCancelButton: true, confirmButtonColor: "#DD6B55", confirmButtonText: "Yes, delete.", cancelButtonText: "No, Cancel.", closeOnConfirm: false, closeOnCancel: false }, function(isConfirm){   if (isConfirm) {  document.myform.action = "actions/deletelecturer.php?page=ViewAllLecturers&PageNo=<?php echo $PNo ?>"; document.myform.submit(); } else {     swal("Cancelled", "It is safe :)", "error"); } });
            };
            ...
            $sql_2 = "SELECT * FROM cp_lecturers WHERE lec_id LIKE '%{$SearchKey}%' OR lec_name LIKE '%{$SearchKey}%'";
            ...
            document.querySelector('#swalt2').onclick = function setDeleteAction2(){
                             swal({   title: "Are you sure?", text: "Do you want to delete.", type: "warning", showCancelButton: true, confirmButtonColor: "#DD6B55", confirmButtonText: "Yes, delete.", cancelButtonText: "No, Cancel.", closeOnConfirm: false, closeOnCancel: false }, function(isConfirm){   if (isConfirm) {  document.myform2.action = "actions/deletelecturer.php?page=ViewAllLecturers&SearchKey=<?php echo $SearchKey ?>"; document.myform2.submit(); } else {     swal("Cancelled", "Your items are safe :)", "error"); } });
                    };
