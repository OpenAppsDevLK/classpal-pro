...
$sql = "SELECT cp_subj_allo.sa_id, cp_subj_allo.sa_stu_student_id, cp_subj_allo.sa_subj_id, cp_subj_allo.sa_lec_id, cp_subj_allo.sa_subj_fee, cp_subj_allo.sa_batch_no, cp_subj_allo.sa_notes, cp_students.stu_studentname, cp_lecturers.lec_id, cp_lecturers.lec_name  FROM `cp_subj_allo` INNER JOIN `cp_students` ON cp_subj_allo.sa_stu_student_id=cp_students.stu_studentID LEFT JOIN `cp_lecturers` ON cp_lecturers.lec_id = cp_subj_allo.sa_lec_id WHERE sa_subj_id=$Subj_ID ORDER BY cp_subj_allo.sa_stu_student_id ASC $limit";
...
?>
          ...
          <h1>
            View Subject Allocated Students
            <small></small>
          </h1>               
        </section>
        ...
        <div class="box-body">
         ...
         <!-- Search Form -->       
                ...
                <form style="margin-bottom: 10px;" role="form" method="get" action="" class="form-inline">
                    ...
                    <input style="margin-top: 10px; width: 260px" class="form-control" type="text" name="SearchKey" value="<?php echo $_GET['SearchKey']; ?>" placeholder="Student ID, Name, Batch No or Lecturer Name"/>
                    ...
                    <a style="margin-top: 10px;" href="index.php?page=BConverter&SubjectID=<?php echo $SubjectID; ?>&SearchKey=<?php echo $SearchKey; ?>" class="btn btn-danger btn-flat <?php echo $btn_BConverter; ?>">B:Converter</a>
      ...
      <h4 class="box-title">Total Students: <?php echo $TotalStudents; ?></h4>
                    ...
                    <tbody>
                      ...
                      <tr>
                         ...
                         <td><?php echo $row['sa_subj_fee']  ?></td>
			...
			<SCRIPT LANGUAGE="JavaScript">
                                    ...
                                    swal({   title: "Are you sure?",   text: "Do you want to delete?",   type: "warning",   showCancelButton: true,   confirmButtonColor: "#DD6B55",   confirmButtonText: "Yes, delete.",   cancelButtonText: "No, Cancel.",   closeOnConfirm: false,   closeOnCancel: false }, function(isConfirm){   if (isConfirm) {  document.myform.action = "actions/deletestuallo.php?page=ViewSubjectAllocatedStudents&SubjectID=<?php echo $SUBJECT_ID; ?>&PageNo=<?php echo $PNo ?>";  document.myform.submit(); } else {     swal("Cancelled", "Your items are safe :)", "error");   } });
 ...
 <?php
                       ...
                       $sql_2 = "SELECT cp_subj_allo.sa_id, cp_subj_allo.sa_stu_student_id, cp_subj_allo.sa_subj_id, cp_subj_allo.sa_lec_id, cp_subj_allo.sa_subj_fee, cp_subj_allo.sa_batch_no, cp_subj_allo.sa_notes, cp_students.stu_studentname, cp_lecturers.lec_id, cp_lecturers.lec_name  FROM `cp_subj_allo` INNER JOIN `cp_students` ON cp_subj_allo.sa_stu_student_id=cp_students.stu_studentID LEFT JOIN `cp_lecturers` ON cp_lecturers.lec_id = cp_subj_allo.sa_lec_id WHERE (cp_subj_allo.sa_stu_student_id LIKE '%{$SearchKey}%' OR cp_students.stu_studentname LIKE '%{$SearchKey}%' OR cp_subj_allo.sa_batch_no LIKE '%{$SearchKey}%' OR cp_lecturers.lec_name LIKE '%{$SearchKey}%') AND cp_subj_allo.sa_subj_id=$Subj_ID ORDER BY cp_subj_allo.sa_stu_student_id ASC";
...
<form name="myform2" action="" method="post">
    ...
    <table id="vas_table2" class="table table-hover table-bordered table-responsive">
			...
			<SCRIPT LANGUAGE="JavaScript">
                                    ...
                                    swal({   title: "Are you sure?",   text: "Do you want to delete?",   type: "warning",   showCancelButton: true,   confirmButtonColor: "#DD6B55",   confirmButtonText: "Yes, delete.",   cancelButtonText: "No, Cancel.",   closeOnConfirm: false,   closeOnCancel: false }, function(isConfirm){   if (isConfirm) {  document.myform2.action = "actions/deletestuallo.php?page=ViewSubjectAllocatedStudents&SubjectID=<?php echo $SUBJECT_ID2; ?>&SearchKey=<?php echo $SKey; ?>";  document.myform2.submit(); } else {     swal("Cancelled", "Your items are safe :)", "error");   } });
