...
$stmt1124 = $db->prepare("SELECT cp_users.id, cp_users.firstname, cp_users.lastname, cp_userpermission.permission_id, cp_userpermission.uid, cp_userpermission.OnOff  FROM `cp_users` INNER JOIN `cp_userpermission` ON cp_users.id = cp_userpermission.uid WHERE cp_userpermission.uid = {$_SESSION['user_id']} AND cp_userpermission.permission_id = 1124" ); 
$stmt1124->bind_result($cp_users_id, $cp_users_firstname, $cp_users_lastname, $cp_userpermission_permission_id, $cp_userpermission_uid, $cp_userpermission_OnOff);
$stmt1124->execute();

while ($stmt1124->fetch()){ 
...
?> 
                    ...
                    <tbody>
                      ...
                      <tr>
                          <td>
                                ...
                                <a href="index.php?page=AssignPermissions&UserID=<?php echo $row['id']; ?>&UserName=<?php echo $row['username']; ?>&PageNo=<?php echo $PNo; ?>&tab=1" class="btn btn-app ">
 ...
 <?php
                ...
                $sql_2 = "SELECT id, username, password, firstname, lastname FROM cp_users WHERE username LIKE '%{$SearchKey}%' OR firstname LIKE '%{$SearchKey}%' ";
