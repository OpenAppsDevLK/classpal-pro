...
// This is your query again , it is for grabbing just one page worth of rows by applying $limit
    $sql = "SELECT cp_attendance.id,cp_attendance.student_id, cp_attendance.date, cp_attendance.att_time, cp_students.stu_studentID, cp_students.stu_studentname, cp_subjects.subj_id, cp_subjects.subj_name, cp_attendance.lec_name  FROM `cp_attendance` LEFT JOIN `cp_students` ON cp_attendance.student_id=cp_students.stu_studentID JOIN `cp_subjects` ON cp_subjects.subj_id = cp_attendance.subj_id ORDER BY cp_attendance.date DESC $limit";
    ...
    if ($cp_userpermission_OnOff == 0) {
        ...
        ?>
                <h1>
                    ...
                    <small>You can see daily students attendance...</small>
                </h1>
            </section>
        ...
        if (!isset($_GET["SearchKey"])) {
            ...
            document.querySelector('#swalt').onclick = function setDeleteAction(){
                                                swal({   title: "Are you sure?", text: "Do you want to delete ?", type: "warning", showCancelButton: true, confirmButtonColor: "#DD6B55", confirmButtonText: "Yes, delete.", cancelButtonText: "No, Cancel.", closeOnConfirm: false, closeOnCancel: false }, function(isConfirm){   if (isConfirm) {  document.myform.action = "actions/deleteattendance.php?page=ViewAllAttendance&PageNo=<?php echo $PNo; ?>"; document.myform.submit(); } else {     swal("Cancelled", "There are safe :)", "error"); } });
                 };
