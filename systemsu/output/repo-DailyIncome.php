...
<html>
  ...
  <body>
        ...
        <div class="row">
          <div class="col-xs-12">
            <h2 class="page-header">
              <i class="fa fa-usd"></i> Report: Daily Income | Subject Name: <?php echo $AS_Sunj_Name; ?> | Date: <?php echo $date; ?>
    ...
    $stmt = $db->prepare("SELECT Pay_stu_studentID, pay_student_name, pay_subj_id, pay_paymentdate, pay_paymentmonth, pay_cos_fee, pay_cos_admi, pay_cos_total FROM `cp_payments` WHERE pay_paymentdate LIKE '%{$date}%' AND pay_subj_id = $Subj_ID ORDER BY pay_paymentdate DESC");
    $stmt->bind_result($Pay_stu_studentID, $pay_student_name, $pay_subj_id, $pay_paymentdate, $pay_paymentmonth, $pay_cos_fee, $pay_cos_admi, $pay_cos_total);
    $stmt->execute();
          ...
          <div class="col-xs-6">
            <div class="table-responsive">
              ...
              <?php
               ...
               $stmt1 = $db->prepare("SELECT SUM(pay_cos_total) FROM cp_payments WHERE pay_paymentdate LIKE '%{$date}%' AND pay_subj_id = $Subj_ID");
