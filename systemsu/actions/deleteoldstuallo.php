...
// if search keyword is seet on URL....
 if(isset($_GET["SearchKey"])){
     ...
     $rowCount = count($_POST["checkbox"]);
            ...
            $stmt = $db->prepare("DELETE FROM cp_oldstudentallo WHERE osa_id='" . $_POST["checkbox"][$i] . "'");
            $stmt->execute(); 
            $stmt->close();
...
header('Location: ../index.php?page=OldStudentAllocations&SubjectID=' . $SUBJECT_ID); 
...
//Jump to the same page after deleteing the image
header('Location: ../index.php?page=OldStudentAllocations&SubjectID='. $SUBJECT_ID . '&PageNo=' . $PNo); 
